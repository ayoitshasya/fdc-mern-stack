    ...
    useEffect(() => {
        const fetchReimbursement = async () => {
          try {
            ...
            const response = await fetch('http://localhost:4000/reimbursement/fetch-reimbursement-by-id', {
              method: 'POST',
              credentials: 'include',
              headers: {
      ...
      // run fetchApplication only when `form` is set and has application_id
useEffect(() => {
    if (!form || !form.application_id) return;
  
    const fetchApplication = async () => {
      try {
        console.log("Fetching application using application_id:", form.application_id);
        ...
        const response = await fetch('http://localhost:4000/application/fetch-application-by-id', {
          ...
          // use the application_id from the fetched reimbursement form
  ...
  return (
    <div className='w-full min-h-screen bg-[#e8e8e8] flex flex-col items-center'>
            ...
            <div className="flex avoid-break w-full">
                ...
                <div className="w-[778px] p-8 font-sans bg-white flex flex-col">
                    <div className='p-2 w-fit self-end mb-5 text-sm' style={{border: "1px solid"}}>
                        Form No. <span className='underline'>{uniqueId}</span> of {academicYear}
                    </div>
        ...
        <div className='flex mt-2 gap-2 bg-white p-2'>
            {form.attachment &&
            <a className="rounded-3xl  bg-gray-300 text-[#b7202e] font-medium p-2 px-4" href={form.attachment}>Download Attached Document</a>
            }
        </div>
        ...
        <div className="mt-4 mb-2 no-print flex gap-4 justify-center">
                    ...
                    <button
                    className="bg-[#B7202E] text-white px-8 py-2 rounded-4xl cursor-pointer"
                    onClick={()=>{navigate(`/reimbursement/review/${form._id}`)}}
                    >
                    Next
                    </button>
        </div>
...
export default ReimbursementPDF_HOD